...
**Documentation maintenance:** When adding new files, modules, extensions, or changing import chains, update `.Codex/architecture.md` to reflect the changes. Keep the file tree, theming section, and tech debt list current. This should happen as part of the work, not as a separate step.
...
See [`.Codex/nix-style-guide.md`](.Codex/nix-style-guide.md) for the full Nix style guide.
...
- [`.Codex/nix-style-guide.md`](.Codex/nix-style-guide.md) — Nix language idioms, module patterns, anti-patterns, and formatting rules
- [`.Codex/neovim-guide.md`](.Codex/neovim-guide.md) — LazyVim framework, plugin management, Lua conventions, nvim.nix integration
- [`.Codex/architecture.md`](.Codex/architecture.md) — Full file map, import chains, overlay strategy, theming, host differences

## Quality Standards

This AGENTS.md prescribes **ideal patterns**, not current state. Existing code may violate these standards — that's tech debt to fix, not precedent to follow. When refactoring or writing new code, always follow these guidelines even if surrounding code doesn't.
